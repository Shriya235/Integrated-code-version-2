...
def wifistats(catch_start,catch_end,file):
    ...
    while i < len(lines):
        if catch_start in lines[i]:
            ...
            i += 1
    if(len(results))==0:
        print("No values are read for "+catch_start.replace(catch_end,' '))
    else:
...
#..............File=Console log file....finding the key parameters for each mac address..........

def each_mac(lis,mac_address,key_para):
    print(mac_address)
    c=0
    for k in key_para:
        c=0    
        for x in lis:
            if mac_address in x and " "+k+" " in x or " "+k+"\n" in x or " "+k.upper()+" " in x or " "+k.upper()+"\n" in x:
                c+=1        
        print(k+" : ",c)
    print("\n")
...
def console(fil,key):
    ...
    for k in key_parameters:
        with open(fil, 'r') as file1:
            for line in file1:    
                if re.findall(p,line) and " "+k+" " in line or " "+k+"\n" in line or " "+k.upper()+" " in line or " "+k.upper()+"\n" in line: 
                    m.append(line)
    
    for sp in m:
        mac.append(re.split(r'[|\s=]\s*', sp))
         
    for a in mac:
        for b in a:
            if re.match("[0-9a-f]{2}([-:]?)[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", b):
                mac_add.append(b)
    
    unique=list(set(mac_add))
    ...
    for x in unique:                         
        ...
        for y in m:
            if x in y:
                many[count-1].append(y)
    for x in range(len(many)):
        each_mac(many[x],unique[x],key_parameters)
...
def uptime(fname):
    ...
    if len(tmp2) !=0 and len(tmp3) != 0:        
        ...
        print("No uptime values available \n")
    infile.close()
